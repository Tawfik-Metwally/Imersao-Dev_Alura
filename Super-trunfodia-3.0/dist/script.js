...
function placarJogo(){
  var divPlacarJogo = document.getElementById("placar")
  var html = "Jogador " + placarJogador + "/" + placarMaquina + " Máquina"
  divPlacarJogo.innerHTML = html
}
...
function jogar() {
    ...
    if (cartas.length == 0){
      ...
      if (placarJogador > placarMaquina){
        htmlResultado = '<p class="resultado-final">Você venceu da Máquina</p>'
      }else if (placarJogador < placarMaquina){
        htmlResultado = '<p class="resultado-final">A Máquina venceu de Você</p>'
      }else{
...
function proximaRodada(){
  ...
  var limparRodada = `<div id="carta-jogador" class="carta"></div>
                    <div id="carta-maquina" class="carta"></div>`
  divCartas.innerHTML = limparRodada
